...
{
    public class MandatoryRulesAddService : IMandatoryRulesService
    {
        ...
        private readonly MovimentTypeEnum movimentType = MovimentTypeEnum.INCLUSÃO;
        public MandatoryRulesAddService(IMandatoryRulesRequestService mandatory, ITaskPanelRequestService task, IEmployeeRequestService employeeRequestService, IMandatoryRulesRuleValidationService mandatoryRulesRuleValidationService)
        {
        ...
        public async Task<MethodFeedback> Validate(Guid hubguid, string aggregator, Benefitinfo beneficio, PersonDB person, string kinship, string typeuser, Guid employeeGuid, string authorization, bool notify)
        {
            ...
            try
            {
                ...
                //filtra pelo tipo de movimentação e hubguid, senao apenas pela movtype
                var ruleList = getRules.Where(x => x.MovimentType.Contains(movimentType) && x.Hubguid == hubguid).ToList();
                if(ruleList == null || ruleList?.Count == 0) ruleList = getRules.Where(x => x.MovimentType.Contains(movimentType)).ToList();
